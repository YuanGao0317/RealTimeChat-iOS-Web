...
- (void)viewDidLoad {
    ...
    [chatCellSettings setSenderBubbleColorHex:@"03A9F4"];
    [chatCellSettings setReceiverBubbleColorHex:@"FF80AB"];
    [chatCellSettings setSenderBubbleNameTextColorHex:@"4E342E"];
    [chatCellSettings setReceiverBubbleNameTextColorHex:@"4E342E"];
...
-(void)viewDidAppear:(BOOL)animated {
    ...
    UIAlertController *alertForUsername = [UIAlertController alertControllerWithTitle:@"Hello!" message:@"What is your name?" preferredStyle:UIAlertControllerStyleAlert];
    
    
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        username = [[alertForUsername.textFields[0] text] copy];
        NSArray *data = @[@{@"username":username}];
        [socket emit:@"add-user" withItems:data];
    }];
    
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:nil];
    
    [alertForUsername addTextFieldWithConfigurationHandler:nil];
    [alertForUsername addAction:okAction];
    [alertForUsername addAction:cancelAction];
    
    [self presentViewController:alertForUsername animated:YES completion:nil];
    
}

-(void) allSocketIOHandlers {
    [socket on:@"connect" callback:^(NSArray* data, SocketAckEmitter* ack) {
        NSLog(@"socket connected");
    }];
    
    [socket on:@"add-user" callback:^(NSArray * data, SocketAckEmitter * ack) {
        NSString *msg = [NSString stringWithFormat:@"%@ has entered the channel!", data[0][@"username"]];
        NSDictionary *dic = @{@"username": @"Robot",
                              @"message": msg};
        [user addObject:data[0][@"username"]];
        
        [self pushMessage:dic];
    }];
    
    [socket on:@"remove-user" callback:^(NSArray * data, SocketAckEmitter * ack) {
        NSString *msg = [NSString stringWithFormat:@"%@ has left the channel!", data[0][@"username"]];
        NSDictionary *dic = @{@"username": @"Robot",
                              @"message": msg};
        
        [user removeObject:data[0][@"username"]];
...
-(void) pushMessage:(NSDictionary *)dic {
    ...
    if (dic[@"message"] && msg.length > 0 && dic[@"username"] != username) {
        iMessage *receiveMessage;
        
        receiveMessage = [[iMessage alloc] initIMessageWithName:dic[@"username"] message:dic[@"message"] time:@"23:14" type:@"other"];
...
- (IBAction)sendMessage:(id)sender
{
    if([self.chatTextView.text length]!=0)
    {
        ...
        [socket emit:@"message" withItems:data];
        
        iMessage *sendMessage;
    
        sendMessage = [[iMessage alloc] initIMessageWithName:username message:self.chatTextView.text time:@"23:14" type:@"self"];
